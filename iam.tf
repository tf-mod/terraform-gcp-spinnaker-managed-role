...
resource "local_file" "credential" {
  ...
  filename = "${path.cwd}/${local.credential_json}"
}

resource "null_resource" "chmod" {
  provisioner "local-exec" {
    command     = "chmod 600 ${local.credential_json}"
...
resource "google_project_iam_member" "spinnaker-managed" {
  ...
  project = "${var.project}"
  role    = "${element(var.spinnaker-managed-roles, count.index)}"
  member  = "serviceAccount:${google_service_account.spinnaker-managed.email}"
}
